...
if execute_on_teleport then
    queue_on_teleport(([[repeat task.wait() until game:IsLoaded();getgenv().execute_on_teleport=%s;script_key='%s';loadstring(readfile("Loader.Lua"))()]]):format((execute_on_teleport and "true") or "false", script_key))
end
